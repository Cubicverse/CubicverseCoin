...
*Setup instructions for a Gitian build of Cubicversecoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Cubicversecoin
...
- [Building Cubicversecoin Core](#building-cubicversecoin-core)
...
Clone the git repositories for cubicversecoin and Gitian.
...
in the cubicversecoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Cubicversecoin Core
----------------

To build Cubicversecoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the cubicversecoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/cubicversecoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/cubicversecoin-project/cubicversecoin
...
For example:
```bash
URL=https://github.com/thrasher-/cubicversecoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit cubicversecoin=${COMMIT} --url cubicversecoin=${URL} ../cubicversecoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit cubicversecoin=${COMMIT} --url cubicversecoin=${URL} ../cubicversecoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit cubicversecoin=${COMMIT} --url cubicversecoin=${URL} ../cubicversecoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the cubicversecoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../cubicversecoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/cubicversecoin
SIGPATH=/some/root/path/cubicversecoin-detached-sigs

./bin/gbuild --url cubicversecoin=${BTCPATH},signature=${SIGPATH} ../cubicversecoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/cubicversecoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/cubicversecoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/cubicversecoin-osx-build.assert
```
...
[cubicversecoin-project/gitian.sigs.ltc](https://github.com/cubicversecoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
